...
In the same code, we only have three players. What happens if the number of players increased to 15? 
How would your function react if it did not have enough cards to deal? 
We don't expect you to solve these edge cases, but if you do come across an elegant solution, be sure to share it with us.
...
You may also enclose any ```print_r()``` statement in ```<pre>print_r($arrayToPrint)</pre>``` so your data structure is *pre-formatted* for the browser and looks nice.